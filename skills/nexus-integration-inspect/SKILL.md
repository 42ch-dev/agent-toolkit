...
1. **Load and check the baseline.** Read the three baseline files. If any of them is missing or fails the id-format/self-check (see `references/capabilities/README.md`), stop and report to the user — never reconstruct capability rows from model memory. If the surveyed repositories are accessible to you, compare the HEAD SHAs recorded in the baseline headers against `git rev-parse --short HEAD` run in each repository's checkout (Nexus checkout against the nexus baseline header; Spoke checkout against the spoke baseline header). Also compare the versions the inspected project pins against the baseline `version` column (for example the project depends on `@42ch/spoke-schemas@0.9.x` while the baseline surveyed `0.10.0`).
