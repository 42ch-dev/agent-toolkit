...
1. **Load and check the baseline.** Read the three baseline files. If the surveyed repositories are reachable from this checkout (`../nexus`, `../spoke`), compare the HEAD SHAs recorded in the baseline headers against `git -C ../nexus rev-parse --short HEAD` / `git -C ../spoke rev-parse --short HEAD`. Also compare the versions the inspected project pins against the baseline `version` column (for example the project depends on `@42ch/spoke-schemas@0.9.x` while the baseline surveyed `0.10.0`).
   - If drift is significant, tell the user the checklist is only as fresh as the baseline and recommend running the refresh flow in `references/capabilities/README.md` before relying on the result. Record the drift in the checklist header. Never edit the baseline yourself — it is read-only.
...
4. **Write the checklist** to `nexus-integration-checklist.md` at the inspected project root (or the user-specified path). One row per baseline capability; copy `id` and `name` verbatim from the baseline. Do not write into `skills/`, `{HARNESS_DIR}`, or any harness path.
...
| `not-integrated` | The capability applies but is unused or not working. | Integration advice required |
| `not-applicable` | Clearly outside this project's use case; must carry a one-line reason. | None required |
...
**Output invariants** — default output file `nexus-integration-checklist.md` at the inspected project workspace root (user may override the path); never write into `skills/` or harness paths. Advice only — do not modify the inspected project. Self-dev advice must not be dropped silently.
